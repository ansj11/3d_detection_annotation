...
def main(debug=True):
    ...
    path = os.path.join(root, 'infos_v1/infos_dict_v1_filter.json')
    ...
    noinside, nocolor, nopoint = 0, 0, 0
    for img_path in infos_dict.keys():
        ...
        if not os.path.exists(img_path.replace('trial_v1', 'trial_v2')):
            image = cv2.imread(img_path, -1)
        else:
            image = cv2.imread(img_path.replace('trial_v1', 'trial_v2'), -1)
        ...
        for track_id, result in track_dict.items():
            ...
            if alpha.max() == 0:
                ...
                nomask += 1
                # image[y0:y1, x0:x1, :][mask[y0:y1, x0:x1]>0,:] = image[y0:y1, x0:x1, :][mask[y0:y1, x0:x1]>0,:]*0.6 + 0.4*np.array([0,255,0])
            ...
            color_dirs, pc_dirs = search_car_brand(sfm_list, brand, subbrand, color)
            if (len(pc_dirs) == 0) and (len(color_dirs) > 0):
                print("not exists color 2dgs...", brand_str, basename)
                nocolor += 1
                cv2.rectangle(image, (x, y), (x+w, y+h), (255,165,0), 2)
                continue
            elif (len(color_dirs) == 0):
                print("not exists 2dgs...", brand_str, basename)
                no2dgs += 1
                cv2.putText(image, subbrand, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 2)
                cv2.rectangle(image, (x, y), (x+w, y+h), (255,0,255), 2)
                continue
            gs_dirs = []
            for pc_dir in pc_dirs:
                if not os.path.exists(pc_dir):
                    print(pc_dir, 'not exists point...', brand_str)
                    nopoint += 1
                    continue
                pc_path = os.path.join(pc_dir, 'point_cloud/iteration_30000/point_cloud.ply')
                gs_dir = '_'.join(pc_dir.split('/')[-2:])
                if not os.path.exists(pc_path):
                    print("not exists point...", pc_path, brand_str, basename)
                    nopoint += 1
                    continue
            ...
            for pc_dir in pc_dirs:
                ...
                gs_dir = '_'.join(pc_dir.split('/')[-2:])
                if not os.path.exists(pc_path):
                    print("not exists...", pc_path, brand_str, basename)
                    continue

                brand_key = pc_dir.split('/')[-2]
                if brand_key not in kpts3d_dict:
                    print("not exists kpts3d...", brand_str, key)
                    no3dkpts += 1
                    continue
        ...
        save_path = img_path.replace('trial_v1', 'show')
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        cv2.imwrite(save_path, image[...,::-1])
        # cv2.imwrite(save_path, render_image[...,::-1])

        # json_path = img_path.replace('trial_v1', 'dicts')[:-3] + 'json'
    ...
    print(total, valid, small, no2dgs, nocolor, nopoint, no3dkpts, noquery, nomask, nopose, nobrand, noinside)
...
def search_car_brand(sfm_list, brand, subbrand, color):
    ...
    sfm_paths1 = []
    for sfm_path in sfm_list:
        if brand in sfm_path and subbrand in sfm_path:
            sfm_paths.append(sfm_path)
            if color in sfm_path:
                sfm_paths1.append(sfm_path)
    
    return sfm_paths, sfm_paths1
