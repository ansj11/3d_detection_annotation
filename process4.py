...
"""process small objects with occlusion"""
def main(mode='bbox', debug=True, padding_pixel=0):
    ...
    json_path = img_path.replace('trial_v2', 'poses_v4')[:-3] + 'json'
...
def GS_Refiner(images, masks, init_cameras, gaussians, device=None, bboxes=None, mode='gspose', scale=1.0):
    ...
    optimizer = optim.AdamW([{'params': [gaussians._delta_R], 'lr': CFG.START_LR*0.05, 'name': 'rotation'}, 
                             {'params': [gaussians._delta_T], 'lr': CFG.START_LR*0.1, 'name': 'translation'}, ], lr=0.0)
    # lr_scheduler = CosineAnnealingWarmupRestarts(optimizer, CFG.MAX_STEPS,
    #                                             warmup_steps=CFG.WARMUP, 
    #                                             max_lr=CFG.START_LR, 
    #                                             min_lr=CFG.END_LR)
    iter_losses = list()
    MAX_STEPS = min(1000, CFG.MAX_STEPS)
    ...
    for iter_step in range(MAX_STEPS):  # 100
        if iter_step == MAX_STEPS * 1 / 2:
            for param_group in optimizer.param_groups:
                if param_group["name"] == "rotation":
                    param_group['lr'] = CFG.START_LR*0.03
        ...
        for idx, (target_img, mask, init_camera) in enumerate(zip(images, masks, init_cameras)):
            ...
            if metric_loss < min_losses[idx]:
                cat = ((render_img.detach()+target_img.detach())*0.5).permute(1,2,0).cpu().numpy() * 255
